...
class DocModifyListener(unohelper.Base, XModifyListener):
    ...
    def __init__(self,):
        ...
        if firstPageStyleName is None \
            or (not firstPageStyleName.startswith("Титульный лист ")
                and not firstPageStyleName.startswith("Первый лист ")):
                    textCursor = doc.Text.createTextCursor()
                    textCursor.PageDescName = "Титульный лист 1"
                    doc.Text.insertControlCharacter(
                        textCursor,
                    ...
                    textCursor.PageDescName = "Первый лист 1"
...
def importEmbeddedModules(*args):
    ...
    if not doc.URL:
        ...
        filePicker.setTitle("Сохранение новой пояснительной записки")
        ...
        filePicker.setDefaultName("Пояснительная записка.odt")
